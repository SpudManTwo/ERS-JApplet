...
public class ThreePlayers2 extends Game implements ActionListener {
	...
	private static JButton cpu2DeckButton; //Not really a button, it just holds the cpu's deck
	...
	private static Queue<Card> computer2Deck; //The Computer's Deck of Cards
	...
	private static TimerTask cpu2ActionTimer;//TimerTask for A.I.
	...
	public ThreePlayers2(Container where,long cpu1,long cpu2,boolean highlighter) { //The Constructor
		...
		for (Card card:deck.deal(18))	 				//Deals the player 26 cards
			playerDeck.add(card);  
		for (Card card:deck.deal(17))	 				//deals the computer 26 cards
...
//		cpu1DeckButton.setIcon(imageHolder.getDeckIcon(Color.RED)); //Uncomment to make it red
		cpu1DeckButton.setBounds(25,0,100,100);
...
//		cpu2DeckButton.setIcon(imageHolder.getDeckIcon(Color.RED)); //Uncomment to make it red
		cpu2DeckButton.setBounds(225,0,100,100);
				...
				public void run() {//Stuff that happens when the computer reacts
					if (hasDelayed&&!isSlappable()&&turn==1) { //Add Card to pot action
						...
						if (computer1Deck.size()>0) { //Makes sure the computer has cards
							...
							ThreePlayers2.cpu1DeckButton.setVisible(false); //If the computer is out of cards
							comp1=false;
							if (comp2) {
								if (!player) {
									endGame("Computer 2");
				...
				public void run() { //Stuff that happens when the computer reacts
					if (hasDelayed&&!isSlappable()&&turn==2) { //Add Card to pot action
						...
						if (computer2Deck.size()>0) { //Makes sure the computer has cards
							...
							ThreePlayers2.cpu2DeckButton.setVisible(false); //If the computer is out of cards
							comp2=false;
							if (player) {
								if (!comp1) {
									endGame("Player");
